...
public class AvailableProjectPresenter {
    ...
    @Nonnull
    private final SyncProjectRequestHandler syncProjectRequestHandler;
    ...
    public AvailableProjectPresenter(@Nonnull AvailableProject project,
                                     ...
                                     @Provided @Nonnull SyncProjectRequestHandler syncProjectRequestHandler,
        ...
        this.syncProjectRequestHandler = checkNotNull(syncProjectRequestHandler);
    }
    ...
    private void addOpenAction() {
        view.addAction(new AbstractUiAction("打开") {
            @Override
            public void execute() {
    ...
    private void addOpenInNewWindowAction() {
        view.addAction(new AbstractUiAction("在新窗口中打开") {
            @Override
            public void execute() {
    ...
    private void addDowloadAction() {
        AbstractUiAction downloadAction = new AbstractUiAction("下载") {
            @Override
            public void execute() {
    ...
    private void addSyncAction() {
        AbstractUiAction syncAction = new AbstractUiAction("同步") {
            @Override
            public void execute() {
                syncProjectRequestHandler.handleProjectSyncRequest(project.getProjectId());
            }
        };
    ...
    private void addTrashAction() {
        ...
        if(project.isInTrash()) {
            trashActionLabel = "从回收站中恢复";
        }
        else {
            trashActionLabel = "移动到回收站";
        }
