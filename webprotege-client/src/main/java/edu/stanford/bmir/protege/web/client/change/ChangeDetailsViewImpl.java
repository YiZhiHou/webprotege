...
public class ChangeDetailsViewImpl extends Composite implements ChangeDetailsView {
    ...
    protected void showRevisionMenu(ClickEvent event) {
        ...
        if (revertRevisionVisible) {
            popupMenu.addItem(new AbstractUiAction(messages.change_revertChangesInRevision() + " " + revision.get().getValue() + " " + messages.change_revertChangesInRevisionSuffix()) {
                @Override
                public void execute() {
    ...
    public void setDiff(List<DiffElement<String, SafeHtml>> diff, int totalChanges) {
        ...
        if (diff.size() < totalChanges) {
            ...
            hiddenChangesCount.setText(messages.extra() + " " + format(hiddenChanges) + " " + messages.change_moreChangesNotShownHere());
