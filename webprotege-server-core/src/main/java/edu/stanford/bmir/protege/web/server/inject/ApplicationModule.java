...
import edu.stanford.bmir.protege.web.server.sync.SyncGeneratorExecutor;
...
public class ApplicationModule {
    ...
    public ExecutorService provideSyncGeneratorExecutorService() {
        // Might prove to be too much of a bottle neck.  For now, this limits the memory we need
        // to generate syncs
        return Executors.newSingleThreadExecutor(r -> {
            Thread thread = Executors.defaultThreadFactory().newThread(r);
            thread.setName(thread.getName().replace("thread", "Sync-Generator"));
            return thread;
        });
    }
