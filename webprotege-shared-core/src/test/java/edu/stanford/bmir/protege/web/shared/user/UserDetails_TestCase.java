...
public class UserDetails_TestCase {
    ...
    private Optional<String> idToken = Optional.of("Id Token");

    @Before
    public void setUp() throws Exception {
        userDetails = new UserDetails(userId, displayName, emailAddress, idToken);
        otherUserDetails = new UserDetails(userId, displayName, emailAddress, idToken);
    }

    @Test(expected = NullPointerException.class)
    public void shouldThrowNullPointerExceptionIf_UserId_IsNull() {
        new UserDetails(null, displayName, emailAddress, idToken);
    }

    @Test(expected = NullPointerException.class)
    public void shouldThrowNullPointerExceptionIf_DisplayName_IsNull() {
        new UserDetails(userId, null, emailAddress, idToken);
    }

    @Test(expected = NullPointerException.class)
    public void shouldThrowNullPointerExceptionIf_EmailAddress_IsNull() {
        new UserDetails(userId, displayName, null, idToken);
    }
    ...
    public void shouldBeEqualToGuestUser() {
        assertThat(UserDetails.getGuestUserDetails(),
                is(new UserDetails(UserId.getGuest(),
                        "Guest",
                        Optional.empty(),
                        Optional.empty())));
    }
